class DataSourceLOGFile implements DataSource, FileBoard, AccelerometerCapableBoard  {
    ...
    final double V_REF = 4.5;

    DataSourceLOGFile(String filePath) {
    ...
    private void parseRow(String[] row, int numChannels) {
        ...
        for (int i = 0; i < numChannels + 1; i++) {
            ...
            if (i == 0) {
                ...
                res = parseInt(row[i]) * BoardCytonConstants.scale_fac_uVolts_per_count;
            }
            line[i] = res;
        }
        // new accel
        if (row.length >= numChannels + 4) {
            accel_x = parseInt(row[numChannels + 1]);
            accel_y = parseInt(row[numChannels + 2]);
            accel_z = parseInt(row[numChannels + 3]);
        }
